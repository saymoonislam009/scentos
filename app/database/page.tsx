    ...
    <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6">
      ...
      <div className="glass rounded-2xl p-4 sm:p-5 mb-6">
        ...
        <div className="flex flex-wrap gap-4">
          <div><p className="mb-2 font-mono text-2xs uppercase tracking-wider text-ash">Season</p><div className="flex flex-wrap gap-1.5">{SEASONS.map(s => <button key={s} onClick={() => setSeason(season === s ? '' : s)} className={`badge border capitalize transition-colors ${season === s ? 'border-gold/50 bg-gold/15 text-gold' : 'border-bone/10 text-ash hover:border-bone/20'}`}>{s}</button>)}</div></div>
          <div><p className="mb-2 font-mono text-2xs uppercase tracking-wider text-ash">Occasion</p><div className="flex flex-wrap gap-1.5">{OCCASIONS.map(o => <button key={o} onClick={() => setOccasion(occasion === o ? '' : o)} className={`badge border capitalize transition-colors ${occasion === o ? 'border-gold/50 bg-gold/15 text-gold' : 'border-bone/10 text-ash hover:border-bone/20'}`}>{o.replace('-', ' ')}</button>)}</div></div>
        </div>
      </div>
