...
export default function NewPartialBottlePage() {
  ...
  const [form, setForm] = useState({ perfumeName:'', brandName:'', daysUsed:'', percentLeft:'70', hasBox:'true', price:'', currency:'BDT', paymentMethod:'both', location:'', contactInfo:'', showContactPublicly:true, description:'' });
  ...
  async function submit(e: React.FormEvent) {
    ...
    const { error: err } = await createClient().from('partial_bottle_listings').insert({
      seller_id: user.id, perfume_name: form.perfumeName, brand_name: form.brandName || null,
      days_used: form.daysUsed ? Number(form.daysUsed) : null, percent_left: Number(form.percentLeft),
      has_box: form.hasBox === 'true', price: Number(form.price), currency: form.currency,
      payment_method: form.paymentMethod, location: form.location || null,
      contact_info: form.contactInfo || null, show_contact_publicly: form.contactInfo ? form.showContactPublicly : false,
  ...
  return (
    <div className="mx-auto max-w-xl px-4 py-16 sm:px-6">
      ...
      <p className="mt-3 flex items-center gap-1.5 text-sm text-ash"><MessageCircle size={13} className="text-gold" />Buyers can always message you in-app — sharing a phone number is optional.</p>
      <form onSubmit={submit} className="glass mt-8 rounded-2xl p-6 sm:p-8">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="block sm:col-span-2"><span className="mb-1.5 block font-mono text-2xs uppercase tracking-wider text-ash">Location</span><input value={form.location} onChange={e => upd('location', e.target.value)} className="input" placeholder="Dhanmondi, Dhaka" /></label>

          <div className="sm:col-span-2 rounded-xl border border-bone/[0.08] p-4">
            <label className="block"><span className="mb-1.5 block font-mono text-2xs uppercase tracking-wider text-ash">Contact info (optional)</span><input value={form.contactInfo} onChange={e => upd('contactInfo', e.target.value)} className="input" placeholder="WhatsApp: 01XXXXXXXXX" /></label>
            <p className="mt-2 text-xs text-ash/70">Leave this blank if you'd rather only communicate through ScentOS messages.</p>
            {form.contactInfo && (
              <label className="mt-3 flex items-center gap-2.5 text-sm text-ash">
                <input type="checkbox" checked={form.showContactPublicly} onChange={e => upd('showContactPublicly', e.target.checked)} className="h-4 w-4 accent-gold" />
                Show this publicly on the listing
                {!form.showContactPublicly && <span className="flex items-center gap-1 text-2xs text-gold"><Lock size={10} />Private — only visible to you</span>}
              </label>
            )}
          </div>
